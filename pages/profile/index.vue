                ...
                <DisclosureButton class="w-full">
                  <div class="flex w-full items-start justify-between text-left text-gray-900">
                    <div>
                      ...
                      <p class="text-sm font-medium">{{ $t('orders.order.status') }}:
                        <span v-if="item.status === 'pending'"
                              class="bg-blue-200 text-blue-500 p-2 rounded">Создан</span>
                        <span v-if="item.status === 'payed'"
                              class="bg-orange-200 text-orange-500 p-2 rounded">Оплачен</span>
                        <span v-if="item.status === 'delivery'"
                              class="bg-purple-200 text-purple-500 p-2 rounded">В доставке</span>
                        <span v-if="item.status === 'delivered'"
                              class="bg-green-200 text-green-500 p-2 rounded">Доставлен</span>
                        <span v-if="item.status === 'cancel'"
                              class="bg-red-200 text-red-500 p-2 rounded">Возврат</span>
                      </p>
                    </div>
                    <div class="text-right">
                      <p class="font-light mb-5">{{ formatDate(item.created_at) }}</p>
                      <p class="font-semibold">{{ $t('orders.order.amount') }}: {{ item.amount }} ₸</p>
                    </div>
                  </div>
                  <div
                      v-if="!open"
                  ...
                  >
                    <td class="whitespace-nowrap py-5 pl-4 pr-3  sm:pl-0">
                      <div class="flex items-center">
                        ...
                        <div class="ml-4">
                          <div class="font-medium text-gray-900">{{ it.product }}</div>
                        </div>
                      </div>
                    </td>
        ...
        <div v-if="addressesList">
          <div
            ...
            <div
              ...
              <p>{{ item.city.name }}, {{ item.address }}, {{ item.entrance }} подъезд, этаж {{ item.floor }},
                кв. {{ item.float }}</p>
              <TrashIcon
